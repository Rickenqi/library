package com.ricky.library.demo.mapper;

import com.ricky.library.demo.domain.BookList;
import com.ricky.library.demo.domain.example.BookListExample;
import java.util.List;
import org.apache.ibatis.annotations.Param;

public interface BookListMapper {
    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table book_list
     *
     * @mbggenerated
     */
    int countByExample(BookListExample example);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table book_list
     *
     * @mbggenerated
     */
    int deleteByExample(BookListExample example);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table book_list
     *
     * @mbggenerated
     */
    int deleteByPrimaryKey(@Param("bookId") String bookId, @Param("listId") String listId);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table book_list
     *
     * @mbggenerated
     */
    int insert(BookList record);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table book_list
     *
     * @mbggenerated
     */
    int insertSelective(BookList record);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table book_list
     *
     * @mbggenerated
     */
    List<BookList> selectByExample(BookListExample example);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table book_list
     *
     * @mbggenerated
     */
    BookList selectByPrimaryKey(@Param("bookId") String bookId, @Param("listId") String listId);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table book_list
     *
     * @mbggenerated
     */
    int updateByExampleSelective(@Param("record") BookList record, @Param("example") BookListExample example);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table book_list
     *
     * @mbggenerated
     */
    int updateByExample(@Param("record") BookList record, @Param("example") BookListExample example);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table book_list
     *
     * @mbggenerated
     */
    int updateByPrimaryKeySelective(BookList record);

    /**
     * This method was generated by MyBatis Generator.
     * This method corresponds to the database table book_list
     *
     * @mbggenerated
     */
    int updateByPrimaryKey(BookList record);
}